...
function swapAmountsConditions(
    ...
    // 2. Check that amountSpecified is consistent with the swap amounts
    if (x0 > 0) {
        // If amountSpecified is positive, we expect a positive amount out for currency1 and a negative amount in for currency0 (if zeroForOne is true)
        require(zeroForOne ? (x0_swap < 0 && x1_swap > 0) : (x0_swap > 0 && x1_swap < 0));
    } else {
        // If amountSpecified is negative, we expect a negative amount out for currency1 and a positive amount in for currency0 (if zeroForOne is true)
        require(zeroForOne ? (x0_swap > 0 && x1_swap < 0) : (x0_swap < 0 && x1_swap > 0));
    }
