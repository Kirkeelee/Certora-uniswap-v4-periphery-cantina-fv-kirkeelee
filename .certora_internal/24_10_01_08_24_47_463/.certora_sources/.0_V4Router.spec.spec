...
rule SwapExactInputBounds (env e) {
    ...
    assert deltaBefore + require_int256(params.amountIn) == deltaAfter; // Ensure delta corresponds to the swap
...
rule ValidPathCheck (env e) {
    ...
    require pathLength == 2; // For multi-hop swaps

    swapExactIn(e, params);
    assert params.path[1].intermediateCurrency != params.path[2].intermediateCurrency; // Prevent circular paths
