...
rule NoPositiveCurrencyDeltaAfterSettlement(method f, env e, calldataarg params) filtered {
    f -> f.selector == sig:settle(Currency, uint256, bool).selector
         || f.selector == sig:settleAll(Currency, uint256).selector} {

    int256 deltaBefore = getCurrencyDeltaExt(params.currency, currentContract);

    f(e, params);

    int256 deltaAfter = getCurrencyDeltaExt(params.currency, currentContract);
    // Ensure the currency delta does not become positive after settlement
