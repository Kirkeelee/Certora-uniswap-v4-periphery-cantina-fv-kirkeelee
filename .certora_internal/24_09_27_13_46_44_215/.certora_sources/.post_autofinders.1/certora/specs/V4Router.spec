...
rule swapExactInSingleRevertingConditions (env e) {
   IV4Router.ExactInputSingleParams params;
    ...
    require swapper == e.msg.sender && e.msg.sender != currentContract;
    ...
    if (params.zeroForOne) {
        ...
        assert (amount1 < params.amountOutMinimum) => lastReverted;
    } else {
        // Swapping Token1 for Token0 (zeroForOne == false)
        assert (amount0 < params.amountOutMinimum) => lastReverted;
    }
}
   ...
   IV4Router.ExactOutputSingleParams params;
    ...
    if (params.zeroForOne) {
        // Swapping Token0 for Token1 (zeroForOne == true)
        assert (amount1 > params.amountInMaximum) => lastReverted;
    } else {
        // Swapping Token1 for Token0 (zeroForOne == false)
        assert (amount0 > params.amountInMaximum) => lastReverted;
    }
