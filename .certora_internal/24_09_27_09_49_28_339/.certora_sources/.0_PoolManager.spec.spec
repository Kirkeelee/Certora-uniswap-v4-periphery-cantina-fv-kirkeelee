...
function swapAmountsConditions(
    ...
    require(x0_swap != 0 || x1_swap != 0, "No swap amounts provided");
    
    /// 2. Check swap direction consistency based on `zeroForOne`
    if (zeroForOne) {
        require(x0_swap > 0 && x1_swap < 0, "Inconsistent swap direction: zeroForOne is true");
    } else {
        require(x0_swap < 0 && x1_swap > 0, "Inconsistent swap direction: zeroForOne is false");
    }

    /// 3. Cannot receive both tokens (x0_swap > 0 && x1_swap > 0)
    require(!(x0_swap > 0 && x1_swap > 0), "Cannot receive both tokens");

    /// 4. Cannot give both tokens (x0_swap < 0 && x1_swap < 0)
    require(!(x0_swap < 0 && x1_swap < 0), "Cannot give both tokens");
