...
rule BurnEffects (env e) {
    ...
    PositionManagerHarness.PoolKey poolKey; PositionManagerHarness.PositionInfo info;
    int24 tickLower = getTickLower(tokenId);
    int24 tickUpper = getTickUpper(tokenId);
    (poolKey, info) = getPoolAndPositionInfo(tokenId);
    bytes32 positionId = calculatePositionKeyEx(tickLower, tickUpper, tokenId);
    address manager;
    bytes25 poolId = getPoolId(tokenId);
    uint128 liquidityBefore = positionLiquidity(manager, poolId, positionId);
    require liquidityBefore !=0;


    burnPosition(e, tokenId, amount0Min, amount1Min, hookData);
   
    uint128 liquidityAfter = positionLiquidity(manager, poolId, positionId);
