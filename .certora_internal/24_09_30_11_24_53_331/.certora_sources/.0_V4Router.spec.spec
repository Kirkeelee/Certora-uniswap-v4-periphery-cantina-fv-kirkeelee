...
Copy code
rule CurrencyDeltaAfterSwaps(method f, env e, calldataarg args) filtered {
    ...
    assert deltaAfterIn <= deltaBeforeIn;  // Consumed or unchanged input currency
    assert deltaAfterOut >= deltaBeforeOut;  // Gained or unchanged output currency
}