...
rule NoNegativeLiquidity (env e, method f, calldataarg args, uint256 tokenId) {
...
rule TickConsistency (env e, method f, calldataarg args, uint256 tokenId) {
    PositionManagerHarness.PoolKey poolKey; PositionManagerHarness.PositionInfo info;
    int24 lowerTickBefore; int24 upperTickBefore; int24 lowerTickAfter; int24 upperTickAfter;

    (poolKey, info) = getPoolAndPositionInfo(tokenId);
    (lowerTickBefore, upperTickBefore) = getTicks(e, info);

    require lowerTickBefore !=0;
    f(e, args);
    
    (lowerTickAfter, upperTickAfter) = getTicks(e, info);
  

    assert lowerTickBefore > upperTickBefore || lowerTickAfter > upperTickAfter;
}