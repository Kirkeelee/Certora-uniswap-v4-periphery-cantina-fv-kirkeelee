...
rule swapExactInSingleRevertingConditions (env e) {
   IV4Router.ExactInputSingleParams params;
    address swapper;
    Conversions.BalanceDelta delta = swapMock(swapper, params.poolKey, params.zeroForOne, params.amountIn, params.sqrtPriceLimitX96);

    swapExactInSingle@withrevert(e, params);

    if (params.zeroForOne) {
        // Swapping Token0 for Token1 (zeroForOne == true)
        assert (Conv.amount1(delta) < params.amountOutMinimum) => lastReverted;
    } else {
        // Swapping Token1 for Token0 (zeroForOne == false)
        assert (Conv.amount0(delta) < params.amountOutMinimum) => lastReverted;
    }
}
   ...
   IV4Router.ExactOutputSingleParams params;
    ...
    require params.amountOut < max_uint128 && amountIn < max_uint128;

    swapExactOutSingle@withrevert(e, params);

     if (params.zeroForOne) {
        // Swapping Token0 for Token1 (zeroForOne == true)
        assert (Conv.amount1(delta) > params.amountInMaximum) => lastReverted;
    } else {
        // Swapping Token1 for Token0 (zeroForOne == false)
        assert (Conv.amount0(delta) > params.amountInMaximum) => lastReverted;
    }
