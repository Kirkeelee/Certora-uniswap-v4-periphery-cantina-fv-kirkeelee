...
rule RouterBalanceAfterSwaps(method f, env e, calldataarg args) filtered {
    ...
    Conversions.Currency currency2
    uint256 balanceBeforecurrency1 = balanceOfCVL(currency1, currentContract);
    uint256 balanceBeforecurrency2 = balanceOfCVL(currency2, currentContract);
    f(e, args);

    uint256 balanceAftercurrency1 = balanceOfCVL(currency1, currentContract);
    uint256 balanceAftercurrency2 = balanceOfCVL(currency2, currentContract);

    assert balanceBeforecurrency1 == balanceAftercurrency1 || balanceBeforecurrency2 == balanceAftercurrency2;
}