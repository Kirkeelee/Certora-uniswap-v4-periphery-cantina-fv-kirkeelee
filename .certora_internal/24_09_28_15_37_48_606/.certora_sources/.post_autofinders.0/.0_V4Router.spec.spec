...
rule ExactOutputBalanceIntegrity (env e) {
   IV4Router.ExactOutputParams params;
   uint256 balanceBefore =balanceOfCVL(params.currencyOut, e.msg.sender);

   swapExactOut(e, params);

   uint256 balanceAfter = balanceOfCVL(params.currencyOut, e.msg.sender);

   assert balanceAfter == (params.amountOut + balanceBefore);
