...
rule SwapExactInputBounds (env e) {
    ...
    assert inputBefore >= inputAfter + params.amountIn; // Ensure swap exact input adheres to input bounds
}
...
rule SwapExactInputSingleBounds (env e) {
    ...
    assert inputBefore >= inputAfter + params.amountIn; // Ensure single swap exact input stays within bounds
}
...
rule SwapExactOutputSingleBounds (env e) {
    ...
    assert outputBefore + params.amountOut >= outputAfter; // Ensure single swap exact output stays within bounds
}