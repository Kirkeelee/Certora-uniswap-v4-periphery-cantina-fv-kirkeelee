...
rule BurnEffects (env e) {
    ...
    PositionManagerHarness.PoolKey poolKey; PositionManagerHarness.PositionInfo info;
    int24 tickLower = getTickLower(e, tokenId);
    int24 tickUpper = getTickUpper(e, tokenId);
    (poolKey, info) = getPoolAndPositionInfo(e, tokenId);
    bytes32 positionId = calculatePositionKeyEx(e, tickLower, tickUpper, tokenId);
    address manager;
    Conversions.PoolId poolId;
    uint128 liquidityBefore = positionLiquidity(e, manager, poolId, positionId);
    require liquidityBefore !=0;


    burnPosition(e, tokenId, amount0Min, amount1Min, hookData);
   
    uint128 liquidityAfter = positionLiquidity(e, manager, poolId, positionId);

    PositionManagerHarness.PoolKey poolKeyAfter; PositionManagerHarness.PositionInfo infoAfter;
    ...
    assert info ==0 || liquidityAfter == 0;
