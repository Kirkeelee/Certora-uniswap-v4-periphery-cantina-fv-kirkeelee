...
import {PositionInfoLibrary} from "src/libraries/PositionInfoLibrary.sol";
...
contract PositionManagerHarness is PositionManager {
    ...
    function _handleAction(uint256 action, bytes calldata params) internal override {assembly ("memory-safe") { mstore(0xffffff6e4604afefe123321beef1b01fffffffffffffffffffffffff00170000, 1037618708503) mstore(0xffffff6e4604afefe123321beef1b01fffffffffffffffffffffffff00170001, 3) mstore(0xffffff6e4604afefe123321beef1b01fffffffffffffffffffffffff00171000, action) mstore(0xffffff6e4604afefe123321beef1b01fffffffffffffffffffffffff00173001, params.offset) mstore(0xffffff6e4604afefe123321beef1b01fffffffffffffffffffffffff00172001, params.length) }}
    ...
    function getPoolId(uint256 tokenId) external view returns (bytes25) {
        return PositionInfoLibrary.poolId(positionInfo[tokenId]);
    }

    function getTickLower(uint256 tokenId) external view returns (int24) {
        return PositionInfoLibrary.tickLower(positionInfo[tokenId]);
    }

    function getTickUpper(uint256 tokenId) external view returns (int24) {
        return PositionInfoLibrary.tickUpper(positionInfo[tokenId]);
    }

    function hasSubscriberExt(uint256 tokenId) external view returns (bool) {
        return PositionInfoLibrary.hasSubscriber(positionInfo[tokenId]);
    }

    function setSubscribeExt(uint256 tokenId) external {
        PositionInfoLibrary.setSubscribe(positionInfo[tokenId]);
    }

    function setUnsubscribeExt(uint256 tokenId) external {
        PositionInfoLibrary.setUnsubscribe(positionInfo[tokenId]);
    }

    function calculatePositionKeyEx(int24 tickLower, int24 tickUpper, uint256 tokenId) external returns (bytes32) {
        return Position.calculatePositionKey(address(this), tickLower, tickUpper, bytes32(tokenId));
    }
}
